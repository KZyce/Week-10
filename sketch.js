...
class Particle {
  ...
  applyPerlinMotion() {
    let yNoise = noise(this.noiseSeed + frameCount * 0.01) - 0.5;
    this.acc.y += yNoise * 0.1;
  }
  ...
  mouseInteraction() {
    ...
    if (distance < 80) { 
      dir.setMag(0.5 + (80 - distance) * 0.02);
      this.applyForce(dir);
    }
  }
  ...
  show() {
    ...
    let hueValue = (this.pos.x + frameCount) % 360; // Cycle through HSB hues
    let particleColor = color(hueValue, 80, 100, this.alpha); 

    
    let sizeNoise = noise(this.noiseSeed + frameCount * 0.02);
    let size = map(sizeNoise, 0, 1, 1, 4); 
